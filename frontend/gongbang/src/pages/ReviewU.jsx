...
import { deleteReviewImage, getCoffeeCafeDetailAPI, getCoffeeCafeDetailReviewCreateAPI, getReview, userAPI } from "../apis/api";
    ...
    return (
        <div className="review">
            <div>
                ...
                <div className="reviewcreate-title-container"><input name = "title" className= "reviewcreate-title"  value={inputs.title} placeholder="제목" onChange={onChange}/>
                </div>
                <div className="listcontainer-info">
                    <div>
                        {/* <div><input name = "type" onChange={onChange} placeholder="타입"/></div> */}
                        <select className ="reviewcreate-type-select"onChange={handleTypeSelect} value={typeSelect}>
                                {typeList.map((item) => { 
                    ...
                    <div>
                        <div><input className ="reviewcreate-type-select" name = "score" onChange={onChange}  value={inputs.score} placeholder="점수"/></div>
                            ...
                            <div>
                                <div className="review-image-list">
                                    ...
                                    <div className="review-image-card" key={index}>
                                        ...
                                        {image.id ? <img src = {image.image}/> :
                                        
                                        <img  src={URL.createObjectURL(image)} alt={`Preview ${index + 1}`} />}

                                         {image.id ? <div><button onClick={() => handleDelete(image.id)}>Del</button> </div> : 
                                            <div>x</div>}
                                    </div>
                                    ))}
                    ...
                    <div>
                        ...
                        <div><textarea className= "reviewcreate_content" name="content" id="" cols="30" rows="10" onChange={onChange} value={inputs.content} ></textarea></div>
                    </div>
